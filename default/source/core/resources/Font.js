...
function Font(url)
{
	...
	this.data = null;
	this.font = null;
	var other_trueType = { "ttf":true, "otf":true, "ttc":true, "otc":true };	
	if(url !== undefined)
	{
		...
		else
		{
			...
			{
				FileSystem.readFile(url,function(_data_)
				{
					var data = JSON.parse(_data_);
					self.data = data;
					self.font = data;
					self.name = data.original_font_information.fullName;
					self.format = "json";					
				});
			}
			else 
			if(this.encoding in other_trueType)
			{
				FileSystem.readFileArrayBuffer(url, function(_data_){
					self.data = _data_;
					self.font = new TTFLoader().parse(_data_);
					self.name = FileSystem.getFileName(url);

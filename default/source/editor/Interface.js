...
Interface.project_extension = ".isp";
...
Interface.initialize = function()
{
	...
	Interface.file.addOption("Save", function()
	{
		...
		{
			Interface.saveProgram(Editor.open_file);
		}
		else
		{
	...
	publish.addOption("Web", function()
	{
		FileSystem.chooseFile(function(files)
		{
			try
			{
				Editor.exportWebProject(files[0].name);
	...
	publish.addOption("Windows", function()
	{
		FileSystem.chooseFile(function(files)
		{
			try
			{
				Editor.exportWindowsProject(files[0].name);
				alert("Project exported");
			}
			catch(e)
			{
	...
	publish.addOption("Linux", function()
	{
		FileSystem.chooseFile(function(files)
		{
			try
			{
				Editor.exportLinuxProject(files[0].name);
				alert("Project exported");
			}
			catch(e)
			{
	...
	Interface.project.addOption("Execute script", function()
	{
		FileSystem.chooseFile(function(files)
		{
			try
			{
				if(files.length > 0)
				{
					var code = FileSystem.readFile(files[0].name);
...
Interface.saveProgram = function(fn)
{	
	try
	{
		var filename_not_exists = !(fn && true);
		if(filename_not_exists)
		{
			fn = "default";
			fn = prompt("Project - Save As",fn);
		}
		fn && Editor.saveProgram(fn);
	}
	catch(e)
	{

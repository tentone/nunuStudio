...
(function e(t, n, r) {
	function s(o, u) {
		...
		var three = require("./threejs/blend_three.js");
		...
		JSBLEND = function JSBLEND(fileuri_or_filedata) {
			var name = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";


			var promise = new Promise(function (res, rej) {
				parser.onParseReady = function (blender_file) {
				...
				if ((typeof fileuri_or_filedata === "undefined" ? "undefined" : _typeof(fileuri_or_filedata)) == "object") {
		...
		module.exports = function (unzipper) {
			...
			var worker = null,
			    FR = new FileReader(),
			    return_object = {
				loadBlendFromArrayBuffer: function loadBlendFromArrayBuffer(array_buffer) {
				...
				loadBlendFromBlob: function loadBlendFromBlob(blob) {
					FR.onload = function () {
				...
				ready: true,
				onParseReady: function onParseReady() {}
			};
    ...
    */
				...
				var BLENDER_FILE = function BLENDER_FILE(AB) {
					this.AB = AB;
				...
				BLENDER_FILE.prototype = {
					...
					primeTypes: function primeTypes(list_of_dna_names) {
						for (var i = 0; i < list_of_dna_names.length; i++) {
							//this.objects[list_of_dna_names[i]] = [];
						}
					},
					getPointer: function getPointer(offset) {
    ...
    */
				...
				function float64Prop(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Float64Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getFloat64(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
				...
				function floatProp(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Float32Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getFloat32(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
						set: function set(float) {
							if (Blender_Array_Length > 1) {} else {
				...
				function intProp(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Int32Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getInt32(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
				...
				function uIntProp(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Uint32Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getUint32(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
						set: function set(int) {
							if (Blender_Array_Length > 1) {} else {
				...
				function shortProp(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Int16Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getInt16(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
						set: function set(float) {
							if (Blender_Array_Length > 1) {} else {
				...
				var uShortProp = function uShortProp(offset, Blender_Array_Length, length) {
					return {
						get: function get() {
							return Blender_Array_Length > 1 ? new Uint16Array(this.__blender_file__.AB, this.__data_address__ + offset, length) : this.__blender_file__.dv.getUint16(this.__data_address__ + offset, this.__blender_file__.template.endianess);
						},
						set: function set(float) {
							if (Blender_Array_Length > 1) {} else {
					...
					return {
						get: function get() {
							if (Blender_Array_Length > 1) {
								var start = this.__data_address__ + offset;
								var end = start;
								var buffer_guard = 0;

								while (this.__blender_file__.byte[end] != 0 && buffer_guard++ < length) {
									end++;
				...
				function pointerProp2(offset) {
					return {
						get: function get() {
							var pointer = this.__blender_file__.getPointer(this.__data_address__ + offset, this.__blender_file__);
							...
							if (link) {
								...
								while (true) {
									...
									var obj = this.__blender_file__.memory_lookup[pointer];
									if (!obj) break;
									results.push(obj);
									j++;
								}
						...
						get: function get() {
							if (Blender_Array_Length > 1) {
								...
								while (j < Blender_Array_Length) {
									var pointer = this.__blender_file__.getPointer(this.__data_address__ + off, this.__blender_file__);
								...
								var _pointer = this.__blender_file__.getPointer(this.__data_address__ + offset, this.__blender_file__);
								return this.__blender_file__.memory_lookup[_pointer];
							}
						},
						set: function set() {}
					};
				...
				var MASTER_SDNA_SCHEMA = function MASTER_SDNA_SCHEMA(version) {
				...
				MASTER_SDNA_SCHEMA.prototype = {
					getSDNAStructureConstructor: function getSDNAStructureConstructor(name, struct) {
						if (struct) {
				...
				var BLENDER_STRUCTURE = function BLENDER_STRUCTURE() {
    ...
    */
				var pointer_function = function pointer_function(pointer) {
					return function () {
				...
				BLENDER_STRUCTURE.prototype = {
					setData: function setData(pointer, _data_offset, data_block_length, BLENDER_FILE) {
					...
					get aname() {
						if (this.id) return this.id.name.slice(2);else return undefined;
					}
				};
				...
				function readFile() {
					...
					while (true) {
						...
						var code_uint = data.getUint32(offset, BIG_ENDIAN);
		...
		var createMaterial = require("./material.js");
		var createTexture = require("./texture.js");
		var createMesh = require("./mesh.js");
		var createLight = require("./light.js");

		function _loadModel(three_scene, model_name, blender_file, cache) {
		...
		function _loadScene(three_scene, blender_file, cache) {
			//build object from blender mesh object
			for (var i = 0; i < blender_file.objects.Object.length; i++) {

				var obj = blender_file.objects.Object[i];

				//Load Lights

				if (obj.type == blender_types.lamp) {

					var light = createLight(obj, blender_file);
		...
		module.exports = function (blender_file) {
			...
			return {
				loadScene: function loadScene(three_scene) {
					return _loadScene(three_scene, blender_file, cache);
				},
				loadModel: function loadModel(model_name) {
					return _loadModel(model_name, blender_file, cache);
				}
			};
		};
		...
		module.exports = function createThreeJSLamp(blend_lamp) {
			...
			var ldata = blend_lamp.data;

			var pos_array = [blend_lamp.loc[0], blend_lamp.loc[2], -blend_lamp.loc[1]];

			var color = ldata.r * 255 << 16 | ldata.g * 255 << 8 | ldata.b * 255 << 0;
			var intesity = ldata.energy;
			var distance = 0;
		...
		module.exports = function () {
			...
			function applyTexture(blender_texture, material) {
				...
				if (blender_texture && blender_texture.tex && blender_texture.tex.ima) {
					...
					var three_texture = createTexture(blender_texture.tex.ima);
			...
			return function createThreeJSMaterial(blend_mat) {
				...
				var at = function at(texture) {
					return applyTexture(texture, material);
				};
		...
		function createThreeJSGeometry(blender_mesh, origin) {
			...
			var verts_array_buff = new Float32Array(verts.length * 3);

			for (var i = 0; i < verts.length; i++) {
				var _vert = verts[i];
				vert_array.push(new THREE.Vector3(_vert.co[0] + origin[0], _vert.co[2] + origin[2], -_vert.co[1] - origin[1]));
			}
			...
			for (var i = 0; i < faces.length; i++) {
				...
				while (indexi < len) {
					...
					face_array.push(new THREE.Face3(face_index_array[0], face_index_array[1], face_index_array[2], face_normals));
		...
		var blender_texture_cache = {};

		module.exports = function createThreeJSTexture(image) {
			var base64 = require("base64-js");
			var parsed_blend_file = image.__blender_file__;
			var texture = null;
			var name = image.aname;
			...
			if (image.packedfile) {
				...
				if (blender_texture_cache[name]) {
					...
					var data = image.packedfile;

					var size = data.size;

					var offset = data.data.__data_address__;

					var raw_data = parsed_blend_file.byte.slice(offset, offset + size);

					var encodedData = base64.fromByteArray(raw_data);
